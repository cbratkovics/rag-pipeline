...
def evaluate_dataset():
    ...
    for metric in all_scores:
        aggregate_scores[metric] = {
            ...
            "std": (
                sum(
                    (x - sum(all_scores[metric]) / len(all_scores[metric])) ** 2
                    for x in all_scores[metric]
                )
                / len(all_scores[metric])
            )
            ** 0.5,
        }
...
def main():
    ...
    with open(output_dir / "report.md", "w") as f:
        ...
        for metric, stats in aggregate_scores.items():
            ...
            f.write(
                f"| {metric.replace('_', ' ').title()} | {stats['mean']:.3f} | {threshold:.1f} | {status} |\n"
            )
