...
def run_load_test():
    ...
    print(f"Starting load test with {NUM_USERS} concurrent users for {TEST_DURATION_SECONDS} seconds")
...
def main():
    ...
    with open(output_dir / "stats_history.csv", "w") as f:
        ...
        f.write(f"{timestamp},{stats.get('total_requests', 0)},{stats.get('successful_requests', 0)},")
